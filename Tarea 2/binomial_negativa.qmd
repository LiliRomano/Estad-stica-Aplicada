...
```{r}
geometrica_binneg <- function(r = 6, p = 0.8, n_sim) { 
  ...
  data_hist <- data.frame(
    valores = c(geometrica_sim, binneg_sim),
    distribucion = rep(c("Geométrica", "Binomial Negativa"), each = n_sim))
...
geometrica_binneg(r = 6, p = 0.8, n_sim = 1000)
```
...
# a) Modele el problema usando la proposición de suma de geométricas
set.seed(42)
sumgeom_f <- replicate(n_sim, {
  sum(rgeom(r, prob = p))})

sim_prob_f <- mean(sumgeom_f)

# b) Calcule el número esperado de tiros fallados 
prob_f <- r * (1-p) / p
desv_std_f <- sqrt(r * (1-p)/(p^2))

cat("Número esperado de tiros fallados (teórico):", round(prob_f, 1),"±", round(desv_std_f, 2), "\n")
cat("Número esperado de tiros fallados (simulación):", round(sim_prob_f,2),"\n\n")
cat("Diferencia:", abs(prob_f - sim_prob_f),"\n\n")

# Gráfica de la Distribución
ggplot(data.frame(Tiros = sumgeom_f), aes(x = Tiros)) +
  geom_histogram(aes(y = after_stat(density)), bins = 30, fill = "tomato", alpha = 0.7) +
  geom_vline(xintercept = prob_f, color = "pink", 
             linetype = "dashed", linewidth = 1) +
  labs(title = "Distribución de tiros fallidos en entrenamiento",
       subtitle = paste("p =", p, ", r =", r, 
                       ". Línea roja: media =", round(prob_f, 1)),
       x = "Número de Tiros fallidos",
       y = "Densidad") +
  theme_minimal()
```
...
# a) ¿Cuál es la probabilidad de tener exactamente 12 pruebas fallidas?
prob_fexacto_12 <- dnbinom(k, size = r, prob = p)

# b) ¿Cuál es la probabilidad de necesitar más de 20 pruebas fallidas?
prob_fmas_20 <- 1 - pnbinom(20, size = r, prob = p)

# c) Si el costo de cada prueba es $10,000, ¿cuál es el costo esperado en pruebas fallidas?
costo_prueba <- 10000
costo_fallidas <- r * (1-p) / p
costo_esperado <- costo_prueba * costo_fallidas

n_sim <- 10000
set.seed(400)

sum_fallidas <- replicate(n_sim, {
  sum(rgeom(r, prob=p))})

prob_sim <- mean(sum_fallidas)

# Gráfica de la Distribución
ggplot(data.frame(iteraciones = sum_fallidas), aes(x = iteraciones)) +
  geom_histogram(aes(y = after_stat(density)), bins = 30, fill = "red", alpha = 0.7) +
  geom_vline(xintercept = costo_fallidas, color = "pink", 
             linetype = "dashed", linewidth = 1) +
  labs(title = "Distribución de pruebas fallidas en el laboratorio",
       subtitle = paste("p =", p, ", r =", r, 
                       ". Línea roja: media =", round(costo_fallidas, 1)),
       x = "Número de Pruebas Fallidas",
       y = "Densidad") +
  theme_minimal()

cat("La probabilidad de tener exactamente 12 pruebas fallidas es (fórmula):", round(prob_fexacto_12, 6), "\n")
cat("La probabilidad de necesitar más de 20 pruebas fallidas (fórmula):", round(prob_fmas_20, 6), "\n")
cat("La media de pruebas fallidas es (simulación):", round(prob_sim, 6), "\n")
cat("El costo esperado en pruebas fallidas es:", round(costo_esperado, 6), "\n")
cat("Diferencia (Teórico-Simulación):", abs(costo_fallidas - prob_sim),"\n\n")
```
