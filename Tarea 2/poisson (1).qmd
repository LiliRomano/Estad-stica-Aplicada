...
-   Variable: $X$ = número de llamadas por minuto
-   Distribución: $X \sim \text{Poisson}(\lambda = 6)$
```{r}
# Parámetro
lambda <- 6
...
# Verificación manual
prob_4_manual <- (lambda^4 * exp(-lambda)) / factorial(4)
cat("\n   Verificación manual:", round(prob_4_manual, 6))

# b) P(X = 0)
prob_0 <- dpois(0, lambda)
cat("\n\nb) P(X = 0) =", round(prob_0, 6))

# c) P(X ≤ 3)
prob_3_o_menos <- ppois(3, lambda)
cat("\n\nc) P(X ≤ 3) =", round(prob_3_o_menos, 6))

# Verificación manual para c)
prob_3_manual <- sum(dpois(0:3, lambda))
cat("\n   Verificación manual:", round(prob_3_manual, 6))
```
**Interpretación de Resultados**

```{r}
# Parámetro lambda (tasa de llamadas por minuto)
lambda_minuto <- 3

prob_4 <- dpois(4, lambda = lambda_minuto)

cat("a) La probabilidad de recibir exactamente 4 llamadas en un minuto es", 
    round(prob_4, 4), "\n")

lambda_30seg <- lambda_minuto * 0.5
prob_0_30seg <- dpois(0, lambda = lambda_30seg)

cat("b) La probabilidad de no recibir ninguna llamada en 30 segundos es", 
    round(prob_0_30seg, 6), "\n")

lambda_hora <- lambda_minuto * 60
cat("c) El número esperado de llamadas en una hora es", 
    lambda_hora, "llamadas\n")

lambda_2min <- lambda_minuto * 2
prob_8_a_12 <- ppois(12, lambda = lambda_2min) - ppois(7, lambda = lambda_2min)

# Cálculo alternativo para verificación
prob_individual <- dpois(8:12, lambda = lambda_2min)
prob_suma <- sum(prob_individual)

cat("d) La probabilidad de recibir entre 8 y 12 llamadas (inclusive) en dos minutos es", 
    round(prob_8_a_12, 6), "\n")
cat("   Verificación (suma individual):", round(prob_suma, 6), "\n\n")
...
```{r aplicacion-calidad}
# Parámetros del control de calidad
lambda_100 <- 1.5  # 1.5 defectos por lote en promedio
productos <- 500
lambda_500 <- (lambda_100/100)*productos

cat("APLICACIÓN 2: CONTROL DE CALIDAD\n")
cat("================================\n")
cat("Parámetro: λ =", lambda_500, "defectos por lote\n\n")
...
cat("1. P(X = 8) =", round(prob_8, 4), "(", round(prob_8*100, 2), "%)\n")

# 2. Probabilidad de tener 10 o más defectos en 500 productos
prob_10_o_mas <- ppois(9, lambda_500)
cat("2. P(X ≥ 10) =", round(prob_10_o_mas, 5), "(", round(prob_10_o_mas*100, 2), "%)\n")

# 3. Número de productos necesarios para que P(x ≥ 1) = 0.95 
lambda_requeridos <- -log(0.05)
productos_necesarios <- (lambda_requeridos / lambda_100) *100
cat("3. P(x ≥ 1) = 0.95, λ =", round(lambda_requeridos, 3), "=> se deben fabricar aproximadamente", round(productos_necesarios),"productos\n")

# 4. Si la empresa mejora el proceso y reduce los defectos a 1.2 por cada 100 productos, ¿cuál sería la nueva probabilidad de tener 10 o más defectos en 500 productos?

lambda_100_mejorado <- 1.2
lambda_500_mejorado <- (lambda_100_mejorado / 100) * productos
prob_10_0_mas_mejorado <- 1 - ppois(9, lambda_500_mejorado)
cat("4.Con mejora (λ =", lambda_500_mejorado, "): P(X ≥ 10) =",
    round(prob_10_0_mas_mejorado, 5), "(", round(prob_10_0_mas_mejorado*100, 2), "%)\n")
...
set.seed(789)
productos <- 500
lambda_500 <- (lambda_100 / 100)
n_lotes_sim <- 1000
lotes_simulados <- rpois(n_lotes_sim, lambda_500)

# Clasificar lotes
clasificacion <- case_when(
  lotes_simulados == 0 ~ "Perfecto",
  lotes_simulados <= 5 ~ "Aceptable",
  lotes_simulados <= 9 ~ "Revisión",
  TRUE ~ "Rechazo"
)
...
ggplot(data_calidad, aes(x = defectos, fill = clasificacion)) +
  geom_histogram(bins = 15, alpha = 0.8, position = "stack") +
  labs(title = "Simulación de Control de Calidad",
       subtitle = paste("n =", format(n_lotes_sim, big.mark = ","), "lotes simulados (λ =", lambda_500, ")"),
       x = "Número de defectos por lote (500 productos",
       y = "Frecuencia",
       fill = "Clasificación") +
  theme_minimal() +
  scale_fill_manual(values = c("Perfecto" = "#2EC", "Aceptable" = "#349",
                              "Revisión" = "#F39", "Rechazo" = "yellow"))
...
# a) P(X = 3)
prob_3_1h <- dpois(3, lambda_hora)
cat("a) P(X = 3) =", round(prob_3_1h, 2.5))

# Verificación manual
prob_3_1h <- (lambda^3 * exp(-lambda)) / factorial(3)
cat("\n   Verificación manual:", round(prob_3_1h, 2.5))

# b) P(X = 0)
lambda_2h <- 2*lambda_hora
prob_0_2h <- dpois(0, lambda_2h)
cat("\n\nb) P(X = 0) =", round(prob_0_2h, 2.5))

# c) P(X < 4)
lambda_4h <- 4*lambda_hora
prob_8_4h <- ppois(8, lambda_4h)
cat("\n\nC)   P(superar capacidad de 8 pacientes en 4 horas):", round(prob_8_4h, 2.5))

# d) Capacidad mínima tal que P(saturación) < 0.10 en 4 horas
# Buscamos el menor k tal que P(X > k) < 0.10  <=> P(X <= k) >= 0.90

k <- 0
while(ppois(k, lambda_4h) < 0.90) {
  k <- k + 1
}
capacidad_minima <- k
prob_saturacion_con_cap_min <- 1 - ppois(capacidad_minima, lambda_4h)
cat("\n\nd)  P(saturación) < 0.10 en 4 horas =",  round(prob_saturacion_con_cap_min, 2.5), "\n") 
```
...
cat("E[X] =", lambda_tcl, "\n")
cat("Var(X) =", lambda_tcl, "\n")
cat("SD(X) =", round(sqrt(lambda_tcl), 7), "\n")
```
...
kable(estadisticas_tcl, digits = 7,
      caption = "Convergencia de estadísticos de la media muestral",
      col.names = c("n", "Media Obs.", "Media Teór.", "Var. Obs.", "Var. Teór.", 
                   "SD Obs.", "SD Teór.", "Error Media", "Error Var.", "Error SD"))
...
# Histogramas de las medias muestrales
ggplot(data_tcl, aes(x = media_muestral)) +
  geom_histogram(aes(y = after_stat(density)), bins = 30, alpha = 0.7, fill = "pink") +
  geom_vline(xintercept = lambda_tcl, color = "#E74", linetype = "dashed", linewidth = 1) +
  facet_wrap(~n_label, scales = "free_y") +
  labs(title = "Distribución de Medias Muestrales (TCL)",
       subtitle = paste("Distribución original: Poisson(λ =", lambda_tcl, "). Línea roja: media teórica"),
       x = "Media muestral",
...
lambda1_exr < 3.5
lambda2_exr <- 2.8
lambda_suma_exr <- lambdal_exr + lambda2_exr
n_sim_suma_exr <- 10000
set.seed(378)

x1_exr <- rpois(n_sim_suma_exr, lambda_suma_exr)
x2_exr <- rpoia(n_sin_suma_exr, lambda2_exr)
suma_exr <- X1_exr + x2_exr
poisson_directa_exr <- rpois(n_sim_suma_exr, lambda_suma_exr)

#Estadistica comparativa
cat("Verificación a la suma de 4 variables Poisson independiente\n")
cat("λ1 =", lambda1_exr,",λ2 =", lambda2_exr, "\n")
...
```{r}
lambda_visitas <- 15
capacidad_de_solicitudes <- 25

cat("Capacidad del servidor Web\n")
cat("============================\n")
cat("Parámetro:  λ =", lambda_visitas,"visitas por minuto\n\n")
cat("capacidad del serVidor:", capacidad_de_solicitudes,"solicitudes\n\n")

# 1. Probabilidad de degradación
prob_degradacion <- 1 - ppois(capacidad_de_solicitudes, lambda_visitas)
cat("1. P(Degradación) = P(X > 25)=", round(prob_mas_25, 5), "(", round(prob_mas_25*100, 2), "%)\n")
# 2.Si se duplica la capacidad del servidor(50 solicitudes)
capacidad_de_solicitudes_doble <- 50
prob_degradacion_doble <- 1 - ppois(capacidad_de_solicitudes_doble, lambda_visitas)
cat("2.Con capacidad duplicada (50 solicitudes):\n")
cat("P(Degradación) =", round(prob_degradacion_doble, 5),"(", round(prob_degradacion_doble*100, 4), "%)\n\n")
# 3.Capacidad óptica del servidor para P(degradación) < 0.01
cap_optima <- qpois(0.99, lambda_visitas)
cat("3.Capacidad óptima (P(degradación) < 0.01) =", cap_optima, "solicitudes simultáneas\n\n")
# 4.Simulación durante 60 minutos
tiempo_min <- 60
visitas_de_simulación <- rpois(tiempo_min, lambda_visitas)
degradacion_min <- sum(visitas_de_simulación > capacidad_de_solicitudes)
porcentaje_degradacion <- (degradacion_min / tiempo_min) * 100
cat("4.Simulación durante 60 minutos :\n")
cat("   Minutos con degradación:", degradacion_min, "de 60 (",
    round(porcentaje_degradacion, 1), "% del tiempo)\n\n")
...
```{r}
# Parámetro de demanda de la tienda
lambda <- 8 

cat("OPTIMIZACIÓN DEL NIVEL DE INVENTARIO\n")
cat("===================================\n")
cat("Demanda promedio diaria (λ):", lambda, "\n\n")

# 1.Probabilidad de quedarse sin existencias con inventario = 12
inventario <- 12
prob_sin_existencia <- 1 - ppois(inventario, lambda)
cat("a) P(Sin existencia) = P(X > 12) =", round(prob_sin_existencia, 5),
    "(", round(prob_sin_existencia*100, 2), "%)\n\n")

# 2.Inventario para que P(Agotamiento) < 0.05
inventario_opt_95 <- qpois(0.95, lambda)
cat("b) Inventario mínimo para P(Agotamiento) < 5%:", inventario_opt_95, "unidades\n\n")

# 3.Costo esperado diario
costo_inventario <- 2   # costo por unidad no vendida
costo_falta <- 15       # costo por venta perdida
costo_adquisicion <- 10 # costo por unidad comprada

# Función para calcular el costo esperado para un inventario dado
calcular_costo_esperado <- function(s, lambda, c_inv, c_falta, c_compra) {
  # Probabilidad para todas las demandas posibles (0 a un valor alto)
  demanda <- 0:40
  prob <- dpois(demanda, lambda)
  
  # Ventas reales = mínimo entre demanda e inventario
  ventas <- pmin(demanda, s)
  
  # Unidades no vendidas y ventas perdidas
  sobrante <- s - ventas
  faltante <- pmax(0, demanda - s)
  
  # Costo total esperado
  costo_esperado <- sum(prob * (s * c_compra + sobrante * c_inv + faltante * c_falta))
  return(costo_esperado)
}
costo_12 <- calcular_costo_esperado(inventario, lambda, costo_inventario, costo_falta, costo_adquisicion)
#Ejemplo
cat("c) Costo esperado diario con inventario de 12 unidades: $", round(costo_12, 2), "\n\n")

# 4.Nivel óptimo de inventario que minimiza el costo total esperado
niveles <- 0:25
costos <- sapply(niveles, calcular_costo_esperado, 
                 lambda = lambda, 
                 c_inv = costo_inventario, 
                 c_falta = costo_falta, 
                 c_compra = costo_adquisicion)

nivel_optimo <- niveles[which.min(costos)]
costo_minimo <- min(costos)

cat("d) Nivel óptimo de inventario:", nivel_optimo, "unidades\n")
cat("   Costo total esperado mínimo: $", round(costo_minimo, 2), "\n")
