...
# Probabilidades importantes
prob_no_defecto <- dbinom(0, n_articulos, p_defecto)
prob_defecto <- dbinom(1, n_articulos, p_defecto)

cat("\nProbabilidades de interés:\n")
cat("P(No defectuosa):", round(prob_no_defecto, 4), "\n")
cat("P(Defectuosa):", round(prob_defecto, 4), "\n")
...
prob_2_defectos <- dbinom(2, n_articulos, p_defecto)

cat("\nProbabilidades de interés:\n")
cat("P(x = 2 defectuosas en 10 piezas)=", round(prob_2_defectos, 6), "\n")
```
...
# Probabilidades
k_resultados <- 0:n_articulos
prob_resultados <- dbinom(k_resultados, n_preguntas, p_acierto)

# Crear data frame
examen_data <- data.frame(
  Resultado = k_resultados,
  Probabilidad = prob_resultados
)

#Tabla
kable(examen_data, 
      caption = paste("Distribución Binomial - Examen (n =", n_preguntas, ", p =", p_acierto, ")"),
      digits = 4,
      col.names = c("Resultado (0=Error,1=Acierto)", "Probabilidad")) %>%
  kable_styling(bootstrap_options = "striped")

# Parámetros estadísticos
media_resultados <- n_preguntas * p_acierto
varianza_resultados <- n_preguntas * p_acierto * (1 - p_acierto)
desv_std_resultados <- sqrt(varianza_resultados)

cat("Parámetros estadísticos:\n")
cat("Media:", media_resultados, "\n")
cat("Varianza:", round(varianza_resultados, 4), "\n")
cat("Desviación estándar:", round(desv_std_resultados, 4), "\n")

cat("Resultados:\n")
cat("a) Modelo: Bernoulli (n=1, p=0.25)\n")
cat("b) Probabilidad de acertar:", p_acierto,":\n")
...
# a) Probalidad de exactamente 18 curas
prob_18_curados <- dbinom(k_curados_18, n_pacientes, p_pacientes)

cat("Solución Ejercicio Medicamento:\n")
cat("a) P(18 curado) =", prob_18_curados, "\n")

#b) Probabilidad de que se curen entre 15 y 19 pacientes.
prob_15_19_curados <- pbinom(19, n_pacientes,
  p_pacientes) - pbinom(14, n_pacientes, p_pacientes)

cat("b) P(entre 15 y 19 curados) =", prob_15_19_curados, "\n")

#c) Pacientes que se espera que se curen
media_curas <- n_pacientes * p_pacientes

cat("c) Pacientes que se espera que se curen =", media_curas, "\n")
```
...
Y <- rbinom(1000, 15, 0.5)

Z <- X + Y

# Comparar con una binomial directa
W <- rbinom(1000, 25, 0.5)

# Dataframes con el mismo nombre de columna
datos_comparacion <- data.frame(
  Valor = Z,
  Tipo = "Suma de Binomiales"
)

datos_comparacion_2 <- data.frame(
  Valor = W,
  Tipo = "Binomial Directa"
)

# Unir en un solo dataframe
datos_final <- rbind(datos_comparacion, datos_comparacion_2)
...
ggplot(datos_final, aes(x = Valor, fill = Tipo)) +
  ...
  scale_fill_manual(values = c("Suma de Binomiales" = "aquamarine",
                               "Binomial Directa" = "chartreuse3")) +
  scale_x_continuous(breaks = seq(0, 25, by = 1)) +
  labs(title = "Comparación: Suma de Binomiales vs Binomial Directa",
       x = "Valor",
       y = "Frecuencia",
       fill = "Distribución") +
  theme_minimal() +
...
ggplot(comp_exr_data, aes(x = Valor, fill = Tipo)) +
  ...
  scale_fill_manual(values = c("Simulada" = "green", "Teórica" = "purple")) +
