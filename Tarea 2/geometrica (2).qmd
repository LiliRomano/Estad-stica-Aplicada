...
# Si se necesita 5 lanzamientos, eso significa 4 fracasos antes (k = 4)
prob_5_lanzamientos <- dgeom(4, p_cara)

cat("a) Probabilidad de necesitar exactamente 5 lanzamientos:", round(prob_5_lanzamientos, 4), "\n")
...
cat("Media (ensayos esperados):", media_moneda, "\n")
cat("Varianza:", varianza_moneda, "\n")
cat("Desviación estándar:", round(desv_std_moneda, 2), "\n\n")

# Si se necesita 5 lanzamientos, eso significa 3 fracasos antes (k = 4)
prob_3_lanzamientos <- dgeom(4, p_cara)

cat("b) Probabilidad de necesitar exactamente 3 lanzamientos:", round(prob_3_lanzamientos, 2), "\n")
...
media_basquet <- (1 - p_encesto) / p_encesto
varianza_basquet <- (1 - p_encesto) / (p_encesto^2)
desv_std_basquet <- sqrt(varianza_basquet)

cat("Tiro Libre - Primer enceste:\n")
cat("Probabilidad de encestar por tiro:", p_encesto, "\n")
cat("Número esperado de intentos hasta el primer enceste:", round(media_basquet, 2), "\n")
cat("Desviación estándar:", round(desv_std_basquet, 2), "\n\n")

prob_tercer_intento <- dgeom(2, p_encesto)

cat("a) Probabilidad de que anote por primera vez en el tercer intento:",
...
media_fallos <- (1 - p_encesto) / p_encesto
varianza_fallos <- (1 - p_encesto) / (p_encesto^2)
desv_std_fallos <- sqrt(varianza_fallos)

# Número esperado de intentos 
media_intentos <- 1 / p_encesto

cat("Tiro Libre - Primer enceste:\n")
cat("Probabilidad de encestar:", p_encesto, "\n")
cat("Media (fallos esperados antes del primer enceste):", round(media_fallos, 2), "\n")
cat("Desviación estándar (fallos):", round(desv_std_fallos, 2), "\n\n")

cat("b) Número esperado de intentos hasta anotar por primera vez:", 
    round(media_intentos, 2), "\n")
...
k_vals <- 1:4
prob_intervalo <- sum(dgeom(k_vals, p_encesto))


cat("Tiro Libre - Primer enceste:\n")
cat("Probabilidad de encestar:", p_encesto, "\n\n")

cat("c) Probabilidad de necesitar entre 2 y 5 intentos (inclusive):",
...
# --- NUEVO: inciso (b) ---
# P(X > 100) = (1 - p)^100
prob_despues_100 <- (1 - p_defecto_geom)^100

cat("b) Probabilidad de que la primera pieza defectuosa aparezca después de la pieza 100:",
    round(prob_despues_100, 6), "\n")
