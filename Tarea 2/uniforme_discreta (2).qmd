...
# Mostrar tabla
kable(dado_data, 
      caption = "Distribución de probabilidad por bolsa con 8 pelotas numeradas",
      align = "cc") %>%
  kable_styling(bootstrap_options = "striped")
```
...
ggplot(dado_data, aes(x = Valor, y = Probabilidad)) +
  geom_col(fill = "tomato", alpha = 0.7, width = 0.6) +
  geom_text(aes(label = round(Probabilidad, 3)), vjust = -0.5) +
  scale_x_continuous(breaks = valores) +
  scale_y_continuous(limits = c(0, 0.20)) +
  labs(title = "Distribución Uniforme Discreta - Dado",
...
set.seed(123)
n_sim <- 500
muestra <- sample(0:4, n_sim, replace = TRUE)
...
ggplot(comp_long, aes(x = Valor, y = Probabilidad, fill = Tipo)) +
  ...
  scale_fill_manual(values = c("Teórica" = "yellow", "Observada" = "pink")) +
  labs(title = paste("Simulación con", n_sim, "observaciones"),
       x = "Valor",
       y = "Probabilidad",
       fill = "Distribución") +
