---
title: "Proyecto EA"
author: "Andrea Zamora, Sarai Briones, Lilian Romano"
format: html
editor: visual
---
...
Este proyecto surge como una aplicación práctica de los conocimientos adquiridos en clase, particularmente de las técnicas econométricas vistas durante el curso, como la regresión lineal simple y la regresión múltiple matricial. A partir de estas herramientas queremos analizar los determinantes socioeconómicos que influyen en la brecha salarial de género en México, incorporando variables que pueden reflejar tanto la dinámica económica del país como las condiciones que tiene el mercado laboral y la implementación de políticas públicas orientadas a la igualdad de género. Este proyecto analiza los determinantes socioeconómicos de la brecha salarial de género en México a través de un enfoque econométrico basado en diversos modelos que buscan explicar estas desigualdades entre hombres y mujeres. El problema central radica en que a pesar de los avances institucionales y las políticas orientadas a promover la igualdad de género, la desigualdad en los salarios entre hombres y mujeres persiste. La investigación inicia con la hipótesis de que la brecha salarial disminuye cuando aumenta la participación femenina en el mercado laboral, se fortalece la presencia de mujeres en puestos directivos y se asignan mayores recursos públicos con perspectiva de género. Los modelos que presentamos incluyen variables que representan el desempeño macroeconomico, políticas gubernamentales, condiciones laborales y características socieconómicas diferenciadas por sexo.
...
# Calcular estadísticas descriptivas a nivel nacional
...
kable(
  ...
  col.names = c("Condición", "Sexo", "Obs.", "Media", "Mediana",
                "Desv. Est.", "Coef. Var.", "Mín.", "Máx."),
  digits = 2,
  caption = "Estadísticas Descriptivas Nacionales sobre Ingresos por Sexo y Condición de Formalidad"
) %>%
  kable_styling(
...
## Evolución de la brecha salarial por año

```{r}
ggplot(datos, aes(x = Año, y = Brecha_salarial)) +
  geom_line(color = "darkred", size = 1) +
  geom_point(size = 2) +
  labs(title = "Evolución de la brecha salarial por año",
       x = "Año",
       y = "Brecha salarial (%)")
```

## Relación entre presupuesto y brecha salarial

```{r}
ggplot(datos, aes(x = Presupuesto, y = Brecha_salarial)) +
  geom_point(aes(size = PIB), alpha = 0.6) +
  geom_smooth(method = "lm", se = TRUE) +
  labs(title = "¿Más presupuesto reduce la brecha salarial?",
       x = "Presupuesto", 
       y = "Brecha salarial (%)")
```
...
```{r}
p1 <- ggplot(datos, aes(x = Año, y = Tasa_participacion)) +
  geom_line(color = "blue") + 
  labs(title = "Tasa de participación")

p2 <- ggplot(datos, aes(x = Año, y = Brecha_salarial)) +
  geom_line(color = "red") + 
  labs(title = "Brecha salarial")

p3 <- ggplot(datos, aes(x = Año, y = Puestos_direccion)) +
...
$$
Brecha\ salarial = PIB + Tasa\ de\ participación + Presupuesto + Puestos\ de\ alta\ dirección + Población\ económicamente\ activa + Ingreso\ de\ hasta\ un\ salario\ mínimo\ hombres + Ingreso\ de\ hasta\ un\ salario\ mínimo\ mujeres
$$

## Regresión lineal simple

```{r}
# MODELO 1: Brecha salarial explicada por Tasa de participación femenina
...
ggplot(datos, aes(x = Tasa_participacion, y = Brecha_salarial)) +
  ...
  geom_text(aes(label = Año), hjust = -0.3, vjust = -0.3, size = 3, color = "gray30") +
  labs(title = "Brecha Salarial vs Tasa de Participación Femenina",
       subtitle = "Análisis temporal 2015-2024",
...
cat("\nMODELO 1: BRECHA SALARIAL ~ TASA PARTICIPACIÓN\n")
...
```{r}
# MODELO 2: Brecha salarial explicada por el número de mujeres con un salario mínimo
...
ggplot(datos, aes(x = Ingreso_min_mujeres, y = Brecha_salarial)) +
  ...
  geom_text(aes(label = Año), hjust = -0.3, vjust = -0.3, size = 3, color = "gray30") +
  labs(title = "Brecha Salarial vs Número de mujeres con un salario mínimo",
       x = "Número de mujeres con un salario mínimo",
       y = "Brecha salarial (%)") +
  theme_minimal() +
  theme(plot.title = element_text(face = "bold", size = 14))
...
cat("\nMODELO 2: BRECHA SALARIAL ~ INGRESO MÍNIMO MUJERES\n")
cat(sprintf("Ecuación estimada:\n"))
...
ggplot(datos, aes(x = Presupuesto, y = Brecha_salarial)) +
  ...
  labs(title = "Brecha Salarial vs Presupuesto",
       subtitle = "Presupuesto destinado a políticas de género",
...
```{r}
# MODELO 5: Brecha salarial explicada por Población Activa (PEA)
...
ggplot(datos, aes(x = Poblacion_activa, y = Brecha_salarial)) +
  ...
  geom_text(aes(label = Año), hjust = -0.3, vjust = -0.3, size = 3, color = "gray30") +
  labs(title = "Brecha Salarial vs Población Económicamente Activa",
...
cat("\nMODELO 5: BRECHA SALARIAL ~ POBLACIÓN ACTIVA\n")
cat(sprintf("Ecuación estimada:\n"))
...
```{r}
# MODELO 6: Brecha salarial explicada por Ingreso Mínimo de Hombres
...
ggplot(datos, aes(x = Ingreso_min_hombres, y = Brecha_salarial)) +
  ...
  geom_text(aes(label = Año), hjust = -0.3, vjust = -0.3, size = 3, color = "gray30") +
  labs(title = "Brecha Salarial vs Ingreso Mínimo de Hombres",
       subtitle = "Número de hombres con salario mínimo",
       x = "Ingreso mínimo - Hombres",
       y = "Brecha salarial (%)") +
  theme_minimal() +
  theme(plot.title = element_text(face = "bold", size = 14),
        plot.subtitle = element_text(color = "gray40"))
...
cat("\nMODELO 6: BRECHA SALARIAL ~ INGRESO MÍNIMO HOMBRES\n")
cat(sprintf("Ecuación estimada:\n"))
...
```{r}
# MODELO 7: Brecha salarial explicada por Puestos de Alta Dirección
...
ggplot(datos, aes(x = Puestos_direccion, y = Brecha_salarial)) +
  ...
  geom_text(aes(label = Año), hjust = -0.3, vjust = -0.3, size = 3, color = "gray30") +
  labs(title = "Brecha Salarial vs Puestos de Alta Dirección",
...
cat("\nMODELO 7: BRECHA SALARIAL ~ PUESTOS DIRECCIÓN\n")
cat(sprintf("Ecuación estimada:\n"))
...
cat("Coeficientes estimados (Regresión Múltiple):\n")
print(betas_mult)
...
cat(sprintf("R^2 (múltiple): %.4f\n", R2_mult))
cat(sprintf("R^2 ajustado (múltiple): %.4f\n", R2_ajustado_mult))
...
plot(Y_mult, y_hat_mult,
     main = "Regresión Múltiple: Brecha Real vs Predicha",
     xlab = "Brecha Salarial Real (%)",
     ylab = "Brecha Salarial Predicha (%)",
     col = "#E63946", pch = 19, cex = 1.5)
abline(a = 0, b = 1, col = "#457B9D", lwd = 2, lty = 2)
grid()
text(Y_mult, y_hat_mult, labels = datos$Año, pos = 3, cex = 0.7, col = "gray30")

# Gráfico: Evolución temporal
ggplot(comparacion_mult, aes(x = Año)) +
  ...
  labs(title = "Regresión Múltiple: Valores Reales vs Predichos",
...
```{r}
cat("\nMODELO LOG-LIN\n")
cat("ln(Brecha_salarial) = β0 + β1*PIB + β2*Ingreso_min_mujeres\n")
cat("\nInterpretación:\n")
...
cat("- Un aumento de 1 unidad en el ingreso mínimo de mujeres se asocia con un cambio de",
...
ggplot(datos, aes(x = Año)) +
  ...
  labs(title = "Modelo Log-Lin: Valores Reales vs Predichos",
       subtitle = "Variables: PIB e Ingreso mínimo mujeres",
       x = "Año",
       y = "Brecha salarial (%)",
       color = "") +
  theme_minimal() +
...
cat("\nMODELO COBB-DOUGLAS\n")
cat("ln(Brecha_salarial) = ln(A) + α*ln(PIB) + β*ln(Presupuesto)\n")
cat("\nInterpretación (elasticidades):\n")
cat("- α =", sprintf("%.4f", coef(modelo_cobb)[2]), 
    ": Un aumento del 1% en el PIB genera un cambio de",
    sprintf("%.4f%%", coef(modelo_cobb)[2]), "en la brecha salarial\n")
cat("- β =", sprintf("%.4f", coef(modelo_cobb)[3]), 
...
cat("      ante un cambio del 1% en X, manteniendo todo lo demás constante)\n")
```

```{r}
# Ingresos mínimos (hombres y mujeres)
...
cat("ln(Brecha_salarial) = ln(A) + α*ln(Ingreso_min_hombres) + β*ln(Ingreso_min_mujeres)\n")
cat("\nInterpretación (elasticidades):\n")
cat("- α =", sprintf("%.4f", coef(modelo_cobb2)[2]), 
    ": Un aumento del 1% en el ingreso mínimo de HOMBRES genera un cambio de",
    sprintf("%.4f%%", coef(modelo_cobb2)[2]), "en la brecha salarial\n")
cat("- β =", sprintf("%.4f", coef(modelo_cobb2)[3]), 
    ": Un aumento del 1% en el ingreso mínimo de MUJERES genera un cambio de",
    sprintf("%.4f%%", coef(modelo_cobb2)[3]), "en la brecha salarial\n")
cat("\nNOTA IMPORTANTE: Este modelo es especialmente relevante para tu hipótesis,\n")
cat("ya que analiza directamente cómo los cambios en los ingresos de cada género\n")
cat("afectan la brecha salarial. Si β (mujeres) es negativo y mayor en magnitud\n")
cat("que α (hombres), indica que aumentar los ingresos de las mujeres reduce\n")
cat("más la brecha que aumentar los ingresos de los hombres.\n")
...
## Modelo de correlación (Detectar colinealidad) (Matriz de correlaciones que detectan colinealidad.)
...
if(nrow(correlaciones_altas) > 0) {
  for(i in 1:nrow(correlaciones_altas)) {
    ...
    if(correlaciones_altas[i,1] < correlaciones_altas[i,2]) {  # Evitar duplicados
      cat(sprintf("%s <-> %s: r = %.4f ⚠️  COLINEALIDAD SEVERA\n", var1, var2, valor))
    }
  }
} else {
...
ggplot(corr_melted, aes(x = Var1, y = Var2, fill = value)) +
  ...
  labs(title = "Matriz de Correlaciones - Detección de Multicolinealidad",
       subtitle = "Valores cercanos a ±1 indican colinealidad",
       x = "", y = "")
```

Nota: La matriz de correlaciones muestra la presencia de multicolinealidad severa entre varias de las variables explicativas del modelo. En particular, las correlaciones se observan en los siguientes indicadores PIB, Presupuesto y Población Activa, lo que indica que estas variables se mueven prácticamente de manera conjunta, esto dificulta que el modelo distinga el efecto individual de cada predictor. La multicolinealidad afecta la estabilidad del modelo y explica el error obtenido durante la regresión múltiple.
...
cat("Interpretación:\n")
cat("  VIF < 5:  ✓ No hay multicolinealidad problemática\n")
cat("  VIF 5-10: ⚠️  Multicolinealidad moderada\n")
cat("  VIF > 10: ❌ Multicolinealidad severa (eliminar variable)\n\n")

# Crear tabla ordenada de VIF
vif_tabla <- data.frame(
  Variable = names(vif_valores),
  VIF = as.numeric(vif_valores),
  Problema = ifelse(vif_valores > 10, "❌ SEVERO", 
                   ifelse(vif_valores > 5, "⚠️  MODERADO", "✓ OK"))
)
...
if(length(vars_severas) > 0) {
  ...
  cat("\nACCIÓN RECOMENDADA: Eliminar estas variables del modelo\n")
}

if(length(vars_moderadas) > 0) {
  cat("\n⚠️  Variables con multicolinealidad MODERADA (VIF 5-10):\n")
...
if(length(vars_severas) == 0 && length(vars_moderadas) == 0) {
  cat("\n✓ No se detectó multicolinealidad problemática en el modelo\n")
}
...
ggplot(vif_tabla, aes(x = reorder(Variable, VIF), y = VIF, fill = Problema)) +
  ...
  scale_fill_manual(values = c("✓ OK" = "#06A77D", 
                               "⚠️  MODERADO" = "#F39C12", 
                               "❌ SEVERO" = "#E63946")) +
  coord_flip() +
  labs(title = "Factor de Inflación de Varianza (VIF)",
       subtitle = "Línea naranja: VIF=5 | Línea roja: VIF=10",
       x = "Variable",
       y = "VIF",
       fill = "Estado") +
  theme_minimal() +
...
## Detección de heterocedasticidad (Prueba de Park)

```{r}
cat("Objetivo: Detectar si la varianza de los errores depende de alguna variable X\n")
cat("Método: Regresión ln(e^2) = α + β*ln(X) + u\n")
cat("Criterio: Si β es significativo (p < 0.05), HAY heterocedasticidad\n\n")

# Modelo múltiple original
...
# Crear variables logarítmicas (evitar log de 0 o negativos)
...
for(var_nombre in names(variables_park)) {
  ...
  resultados_park <- rbind(resultados_park, data.frame(
    ...
    P_value = p_value,
    R2 = r2,
    Heterocedasticidad = ifelse(p_value < 0.05, "❌ SÍ", "✓ NO")
  ))
  ...
  cat(sprintf("  R^2 = %.4f\n", r2))
  if(p_value < 0.05) {
    cat(sprintf("  ❌ HAY heterocedasticidad relacionada con %s\n", var_nombre))
  } else {
    cat(sprintf("  ✓ NO hay heterocedasticidad relacionada con %s\n", var_nombre))
  }
}
...
if(length(vars_problematicas) > 0) {
  cat("\n❌ VARIABLES CON HETEROCEDASTICIDAD DETECTADA:\n")
  for(v in vars_problematicas) {
    cat(sprintf("   - %s\n", v))
  }
  cat("\n📋 RECOMENDACIONES:\n")
  cat("   1. Transformar la variable dependiente: usar log(Brecha_salarial)\n")
  cat("   2. Transformar las variables independientes problemáticas\n")
  cat("   3. Usar errores estándar robustos (HC)\n")
  cat("   4. Aplicar Mínimos Cuadrados Ponderados (WLS)\n")
} else {
  cat("\n✓ NO se detectó heterocedasticidad en ninguna variable\n")
...
# Gráfico 1: Tasa de participación
ggplot(datos, aes(x = log_tasa_participacion, y = residuals2)) +
  geom_point(color = "#E63946", size = 3, alpha = 0.7) +
  geom_smooth(method = "lm", se = TRUE, color = "#457B9D", fill = "#457B9D", alpha = 0.2) +
  labs(title = "Prueba de Park: Tasa de Participación Femenina",
       subtitle = sprintf("β = %.4f, p-value = %.4f", 
...
ggplot(datos_park_largo, aes(x = Log_Valor, y = residuals2)) +
  ...
  labs(title = "Prueba de Park: Análisis por Variable",
...
ggplot(resultados_park, aes(x = reorder(Variable, -P_value), y = P_value, 
  ...
  scale_fill_manual(values = c("✓ NO" = "#06A77D", "❌ SÍ" = "#E63946")) +
  coord_flip() +
  labs(title = "Prueba de Park: P-values por Variable",
       subtitle = "Línea roja: nivel de significancia α = 0.05",
       x = "Variable",
       y = "P-value",
       fill = "Heterocedasticidad") +
  theme_minimal() +
...
Aunque la matriz de correlaciones muestra que hay multicolinealidad entre varias variables, este resultado no contradice la prueba de Park. Ambas técnicas evalúan supuestos diferentes del modelo de regresión: la matriz de correlaciones detecta relaciones excesivamente fuertes entre los predictores, lo cual afecta la estabilidad de los coeficientes y la estimación matricial; en cambio, la prueba de Park analiza la varianza de los errores y determina si existe heterocedasticidad. Que no se haya detectado heterocedasticidad significa que los residuos presentan varianza constante, pero ello no implica ausencia de multicolinealidad.
...
cat("Objetivo: Probar si los residuos siguen una distribución normal\n")
cat("H0: Los residuos siguen una distribución normal\n")
cat("H1: Los residuos NO siguen una distribución normal\n")
cat("Criterio: Si p-value > 0.05, NO rechazamos H0 (residuos normales) ✓\n\n")

residuos <- residuals(modelo_completo)
ad_test <- ad.test(residuos)

cat("\nRESULTADOS\n")
cat(sprintf("Estadístico A = %.4f\n", ad_test$statistic))
cat(sprintf("p-value = %.6f ", ad_test$p.value))

if(ad_test$p.value > 0.05) {
  cat("✓ Los residuos SON normales\n")
  cat("\nInterpretación:\n")
  cat("  - Se cumple el supuesto de normalidad\n")
  cat("  - Las pruebas t y F son confiables\n")
  cat("  - Los intervalos de confianza son válidos\n")
} else {
  cat("❌ Los residuos NO son normales\n")
  cat("\nInterpretación:\n")
  cat("  - Puede haber valores atípicos en los datos\n")
  cat("  - La distribución puede ser asimétrica o tener colas pesadas\n")
  cat("  - Las inferencias estadísticas pueden no ser confiables\n")
  cat("\nSoluciones sugeridas:\n")
  cat("  1. Identificar y tratar valores atípicos\n")
  cat("  2. Transformar la variable dependiente (log, raíz cuadrada)\n")
  cat("  3. Usar métodos robustos o bootstrap\n")
  cat("  4. Aumentar el tamaño de muestra si es posible\n")
}
...
cat("         (No hay correlación entre residuos y variables independientes)\n\n")
cat("H0: E(u|X) = 0  (Los errores son independientes de las X's)\n")
cat("H1: E(u|X) ≠ 0  (Hay correlación entre errores y X's)\n\n")
...
cat("\n--- INTERPRETACIÓN ---\n")
cat("Si p-value > 0.05: Los residuos son independientes de esa variable ✓\n")
cat("Si p-value < 0.05: Hay correlación (viola CIA) ❌\n\n")

# Contar cuántas variables cumplen CIA
...
if(cumple_CIA == total_vars) {
  cat("✓ EL MODELO CUMPLE EL SUPUESTO CIA\n")
  cat("\nSignificado:\n")
  cat("  - Los errores NO están correlacionados con las variables independientes\n")
  cat("  - E(u|X) = 0 se cumple\n")
  cat("  - Los estimadores son INSESGADOS\n")
  cat("  - El modelo está bien especificado\n")
} else {
  cat("❌ EL MODELO VIOLA EL SUPUESTO CIA\n")
...
# Gráfico: Residuos vs cada variable independiente
par(mfrow = c(3, 3))
for(i in 1:ncol(X_vars)) {
...
# Gráfico resumen de correlaciones
library(ggplot2)
ggplot(correlaciones_CIA, aes(x = reorder(Variable, abs(Correlacion)), 
                               y = Correlacion, fill = Resultado)) +
  geom_col(alpha = 0.8) +
  geom_hline(yintercept = 0, linetype = "solid", color = "black", size = 1) +
  scale_fill_manual(values = c("✓ Independiente" = "#06A77D", 
                               "❌ Correlacionado" = "#E63946")) +
  coord_flip() +
  labs(title = "Test CIA: Correlación Residuos vs Variables Independientes",
       x = "Variable",
       y = "Correlación con residuos",
       fill = "Estado") +
  theme_minimal() +
  theme(plot.title = element_text(face = "bold", size = 14),
        legend.position = "bottom")

# Test adicional: Regresión auxiliar (más formal)
cat("\n\n=== TEST FORMAL CIA (Regresión Auxiliar) ===\n")
cat("Regresión: residuos ~ X1 + X2 + ... + Xk\n")
cat("H0: Todos los coeficientes = 0 (CIA se cumple)\n")
cat("H1: Al menos un coeficiente ≠ 0 (CIA se viola)\n\n")

# Regresión de residuos sobre las X's
...
if(p_value_global > 0.05) {
  cat("✓ CIA se CUMPLE (globalmente)\n")
  cat("   Los residuos son conjuntamente independientes de todas las X's\n")
} else {
  cat("❌ CIA se VIOLA (globalmente)\n")
  cat("   Hay correlación conjunta entre residuos y X's\n")
}

cat("\n\nR^2 de la regresión auxiliar:", sprintf("%.4f", summary(modelo_CIA)$r.squared))
...
# ANOVA paso a paso
datos_año <- datos |> select(Brecha_salarial, Año)
media_brecha <- mean(datos$Brecha_salarial, na.rm = TRUE)

datos_año_grupos <- datos_año |> 
  group_by(Año) |> 
  summarise(
    n_i = n(), 
...
datos_año_grupos <- datos_año_grupos |> 
  ungroup() |> 
...
k <- nlevels(datos$Año)
n <- nrow(datos)
SSB <- sum(datos_año_grupos$SSB, na.rm = TRUE)
MSB <- SSB / (k - 1)

datos <- left_join(datos, select(datos_año_grupos, Año, media_grupo), by = "Año")
...
cat("\nANOVA Manual: Brecha_salarial ~ Año\n")
cat("SSB (Between):", SSB, "\n")
cat("MSB:", MSB, "\n")
cat("SSW (Within):", SSW, "\n")
cat("MSW:", MSW, "\n")
...
anova_año <- aov(Brecha_salarial ~ Año, data = datos)
summary(anova_año)
...
# Boxplot: Brecha salarial por Año
ggplot(datos, aes(x = Año, y = Brecha_salarial, fill = Año)) +
  geom_boxplot() +
  theme_minimal() +
  labs(title = "Distribución de Brecha Salarial por Año",
       x = "Año", y = "Brecha Salarial") +
  theme(legend.position = "none")


# Ejemplo con Tukey HSD para comparaciones múltiples
if(!is.na(valor_p) && valor_p < 0.05) {
  tukey_año <- TukeyHSD(anova_año)
  print(tukey_año)
  plot(tukey_año)
...
cat("\n1. Brecha_salarial ~ Año:\n")
print(summary(anova_año))
...
Nos quedamos sólo con PIB, Tasa de participación, Puestos de dirección y Población económicamnete activa ya que estas variables no son problemáticas.

$$
Brecha\ salarial = PIB + Tasa\ de\ participación + Puestos\ de\ alta\ dirección + Población\ económicamente\ activa
$$
...
if(length(vars_moderadas) > 0) {
  ...
  cat("\nACCIÓN RECOMENDADA: Considerar eliminar o mantener bajo supervisión\n")
}
...
if(nrow(significativos) > 0) {
  for(i in 1:nrow(significativos)) {
    ...
    if(var_name != "(Intercept)") {
      ...
      if(coef_val > 0) {
        cat(sprintf("   → Al aumentar %s, la Brecha Salarial AUMENTA\n", var_name))
      } else {
        cat(sprintf("   → Al aumentar %s, la Brecha Salarial DISMINUYE\n", var_name))
      }
    }
  }
} else {
...
nombres_variables <- c("PIB", "Tasa de Participación Femenina", 
                       "Puestos de Dirección (Mujeres)", 
...
grid()
text(x = datos$log_PIB, y = datos$residuals2, labels = datos$Año, 
     pos = 3, cex = 0.7, col = "gray30")

# Tasa de Participación
plot(datos$log_tasa_participacion, datos$residuals2,
     main = "Prueba de Park: Tasa Participación",
     xlab = "log(Tasa Participación)",
     ylab = "Residuos^2",
...
plot(datos$log_puestos_direccion, datos$residuals2,
     main = "Prueba de Park: Puestos Dirección",
     xlab = "log(Puestos Dirección)",
     ylab = "Residuos^2",
...
plot(datos$log_poblacion_activa, datos$residuals2,
     main = "Prueba de Park: Población Activa",
     xlab = "log(Población Activa)",
     ylab = "Residuos^2",
...
# Gráfico comparativo de coeficientes Beta
ggplot(resultados_park, aes(x = reorder(Variable, abs(Beta)), y = Beta, 
  ...
  labs(title = "Prueba de Park: Coeficientes Beta",
       subtitle = "Magnitud de la relación entre varianza del error y cada variable",
       x = "Variable",
       y = "Coeficiente β",
       fill = "Heterocedasticidad") +
  theme_minimal() +
...
ggplot(datos_park_facet, aes(x = Valor_log, y = residuals2)) +
  ...
  labs(title = "Prueba de Park: Residuos^2 vs log(Variables)",
       subtitle = "Detección de heterocedasticidad por variable",
...
hist(residuos, breaks = 10, col = "#E63946", border = "white",
     main = "Distribución de Residuos (Modelo NUEVO)",
     xlab = "Residuos", ylab = "Frecuencia", prob = TRUE)
curve(dnorm(x, mean = mean(residuos), sd = sd(residuos)),
      add = TRUE, col = "#457B9D", lwd = 3)
legend("topright", legend = "Distribución Normal Teórica", 
...
if(cumple_CIA == total_vars) {
  cat("✓ EL MODELO CUMPLE EL SUPUESTO CIA\n")
  cat("\nSignificado:\n")
  cat("  - Los errores NO están correlacionados con las X\n")
  cat("  - E(u|X) = 0 se cumple\n")
  cat("  - Los estimadores son INSESGADOS\n")
} else {
  cat("❌ EL MODELO VIOLA EL SUPUESTO CIA\n")
  ...
  cat("\nSignificado:\n")
  cat("  - Los errores están correlacionados con algunas X's\n")
  cat("  - El modelo puede estar MAL ESPECIFICADO o tener ENDOGENEIDAD\n")
}

# GRÁFICOS: Residuos vs X
par(mfrow = c(2, 2))
for(i in 1:ncol(X_vars)) {
  plot(X_vars[,i], residuos,
       main = paste("Residuos vs", names(X_vars)[i]),
...
cat("Datos después de limpiar:", nrow(datos_clean), "\n\n")
...
if(!is.na(anova_pib_summary[[1]]$`Pr(>F)`[1]) && anova_pib_summary[[1]]$`Pr(>F)`[1] < 0.05) {
  cat("\nComparaciones múltiples para PIB:\n")
...
if(!is.na(anova_tasa_summary[[1]]$`Pr(>F)`[1]) && anova_tasa_summary[[1]]$`Pr(>F)`[1] < 0.05) {
  cat("\nComparaciones múltiples para Tasa de Participación:\n")
...
if(!is.na(anova_puestos_summary[[1]]$`Pr(>F)`[1]) && anova_puestos_summary[[1]]$`Pr(>F)`[1] < 0.05) {
  cat("\nComparaciones múltiples para Puestos de Dirección:\n")
...
if(!is.na(anova_poblacion_summary[[1]]$`Pr(>F)`[1]) && anova_poblacion_summary[[1]]$`Pr(>F)`[1] < 0.05) {
  cat("\nComparaciones múltiples para Población Activa:\n")
...
resultados <- data.frame(
  ...
  Significativo = c(
    ifelse(summary(anova_pib)[[1]]$`Pr(>F)`[1] < 0.05, "Sí ***", "No"),
    ifelse(summary(anova_tasa)[[1]]$`Pr(>F)`[1] < 0.05, "Sí ***", "No"),
    ifelse(summary(anova_puestos)[[1]]$`Pr(>F)`[1] < 0.05, "Sí ***", "No"),
    ifelse(summary(anova_poblacion)[[1]]$`Pr(>F)`[1] < 0.05, "Sí ***", "No")
  )
)

print(resultados)

cat("\n*** p < 0.05 indica que la variable SÍ tiene efecto significativo en la Brecha Salarial\n")
```

# Regresión simple entre el número de hombres con un salario mínimo vs el número de mujeres con un salario mínimo
...
cat("\nINTERPRETACIÓN DEL MODELO\n")
cat("Variable dependiente (Y): Ingreso mínimo de mujeres\n")
cat("Variable independiente (X): Ingreso mínimo de hombres\n\n")
...
cat("   Cuando el ingreso mínimo de hombres es 0, el ingreso mínimo de mujeres\n")
cat(sprintf("   sería de %.4f unidades (interpretación teórica)\n\n", intercepto))

cat(sprintf("β₁ (Pendiente) = %.4f\n", pendiente))
cat("   Por cada unidad adicional en el ingreso mínimo de hombres,\n")
cat(sprintf("   el ingreso mínimo de mujeres cambia en %.4f unidades\n", pendiente))

if(pendiente > 0 && pendiente < 1) {
  cat("\n   ⚠️  HALLAZGO IMPORTANTE:\n")
  cat("   La pendiente es positiva pero MENOR a 1, esto indica que:\n")
  cat("   - Los ingresos de mujeres y hombres están positivamente relacionados\n")
  cat("   - PERO los ingresos de mujeres crecen MENOS que los de hombres\n")
  cat("   - Esto refleja la BRECHA SALARIAL de género\n")
  cat(sprintf("   - Por cada peso que aumenta el ingreso de hombres,\n"))
  cat(sprintf("     el ingreso de mujeres aumenta solo %.2f pesos\n", pendiente))
} else if(pendiente >= 1) {
  cat("\n   Los ingresos de mujeres crecen al mismo ritmo o más que los de hombres\n")
} else {
  cat("\n   ⚠️  Relación negativa o inusual\n")
}

cat(sprintf("R^2 = %.4f\n", r_squared))
cat(sprintf("   El ingreso mínimo de hombres explica %.2f%% de la variabilidad\n", 
            r_squared * 100))
cat(sprintf("   en el ingreso mínimo de mujeres\n\n"))
...
if(p_value < 0.01) {
  cat("***\n   La relación es ALTAMENTE SIGNIFICATIVA (p < 0.01)\n")
} else if(p_value < 0.05) {
  cat("**\n   La relación es SIGNIFICATIVA (p < 0.05)\n")
} else if(p_value < 0.10) {
  cat("*\n   La relación es MARGINALMENTE SIGNIFICATIVA (p < 0.10)\n")
} else {
  cat("\n   La relación NO es estadísticamente significativa (p > 0.10)\n")
}

# Calcular la brecha implícita
brecha_implicita <- (1 - pendiente) * 100
cat(sprintf("\n--- BRECHA SALARIAL IMPLÍCITA ---\n"))
cat(sprintf("Brecha = %.2f%%\n", brecha_implicita))
cat("   Esto significa que, en promedio, las mujeres ganan %.2f%% menos\n", brecha_implicita)
cat("   que los hombres en términos de salario mínimo.\n")

# Gráfico principal: Dispersión con línea de regresión
ggplot(datos, aes(x = Ingreso_min_hombres, y = Ingreso_min_mujeres)) +
  ...
  geom_text(aes(label = Año), hjust = -0.3, vjust = -0.3, size = 3, color = "gray30") +
  labs(title = "Ingreso Mínimo: Mujeres vs Hombres",
       subtitle = sprintf("Y = %.2f + %.2f*X | R^2 = %.4f | p = %.4f", 
                         intercepto, pendiente, r_squared, p_value),
       x = "Ingreso mínimo - Hombres",
       y = "Ingreso mínimo - Mujeres",
       caption = "Línea punteada: igualdad perfecta (45°)") +
  theme_minimal() +
  theme(plot.title = element_text(face = "bold", size = 14),
...
ggplot(datos, aes(x = Año)) +
  ...
  labs(title = "Evolución Temporal: Ingreso Mínimo por Género",
       subtitle = "2015-2024",
       x = "Año",
       y = "Número de personas con salario mínimo",
       color = "Género") +
  theme_minimal() +
  theme(plot.title = element_text(face = "bold", size = 14),
        legend.position = "bottom")

# Gráfico de la brecha por año
ggplot(predicciones, aes(x = Año, y = Brecha_real)) +
  ...
  labs(title = "Brecha Salarial de Género por Año",
...
if(pendiente < 1 && p_value < 0.05) {
  cat("✓ HALLAZGO CLAVE: Existe una relación significativa y positiva,\n")
  cat("  PERO la pendiente menor a 1 confirma que las mujeres reciben\n")
  cat("  sistemáticamente menos que los hombres.\n")
}
```
...
B. La brecha salarial no se ha reducido significativamente a lo largo de los años 2015- 2024, tuvo una caida significativa en la pandemia, reforzando la declaración anterior donde a menor ingreso menor brecha.

C. A partir del análisis de las variables de manera independiente se concluye que la variable que mejor explica la Brecha Salarial es la Población Económicamente Activa seguida por la Tasa de Participación, tienen una relación positiva. La única variable con una relación negativa (mientras aumenta, la Brecha Salarial disminuye) son Mujeres en Puestos Directivos pero el error estándar es muy grande.
...
E. Este nuevo modelo presenta una mejora, ahora el modelo es significativo (p-valor = 0.02441) . Con este nuevo modelo Mujeres en Puestos Directivos es la variable más significativa contradiciendose con las regresiones simples hechas al principio del proyecto.

F. Los modelos son homocedásticos.

## Conclusiones

El desarrollo de este análisis nos permitió identificar los factores socioeconómicos que influyen en la brecha salarial de género en México además de evaluar la consistencia estadística de distintos modelo y aunque varias variables mostraron relaciones significativas de manera individual nuestro modelo enfrentó problemas de colinealidad que limitaron su capacidad explicativa lo que obligó a replantear su estructura y a estimar versiones alternativas con mejor estabilidad estadística.

Con el desarrollo de este trabajo se logra evidenciar los fenómenos sociales, como la brecha salarial de género que pueden ser estudiados mediante herramientas técnicas estadísticas y econometricas, permitiendo cuantificar relaciones que muchas veces se perciben únicamente desde una perspectiva social o política y la brecha salarial que es un problema real y persistente que requiere un análisis más riguroso donde se pueda sustentar la toma de decisiones en materia de igualdad laboral.
...
El estadístico de Anderson-Darling. (s/f). Minitab.com. Recuperado el 21 de noviembre de 2025, de https://support.minitab.com/es-mx/minitab/help-and-how-to/statistics/basic-statistics/supporting-topics/normality/the-anderson-darling-statistic/

Que es la Colinealidad y Multicolinealidad y como saber si nuestras variables son colineales o multicolineales entre sí. (s/f). Com.co. Recuperado el 21 de noviembre de 2025, de https://siepsi.com.co/2024/02/23/que-es-la-colinealidad-y-multicolinealidad-y-como-saber-si-nuestras-variables-son-colineales-o-multicolineales-entre-si/
...
Claudia, M., & Corichi García, S. (s/f). PRESUPUESTOS CON PERSPECTIVA DE GÉNERO. Gob.mx. Recuperado el 22 de noviembre de 2025, de https://www.asf.gob.mx/uploads/54_Informes_y_publicaciones/PPG_CC08ABR19.pdf
...
Axtle Ortiz, M. Á., & Caro-Guzmán, L. (2017). La presencia de las mujeres en posiciones directivas. The Anáhuac journal, 17(2), 53–92. https://doi.org/10.36105/theanahuacjour.2017v17n2.03

De hogares y su población, D. P., & De ingresos por trabaj o, S. G. (s/f). Ingresos en los hogares. Org.mx. Recuperado el 22 de noviembre de 2025, de https://www.inegi.org.mx/contenidos/productos/prod_serv/contenidos/espanol/bvinegi/productos/historicos/2104/702825496661/702825496661_18.pdf
...
Gobierno de México. (2015-2025). Presupuesto de Egresos de la Federación para el ejercicio fiscal de \[2015-2025\]. Diario Oficial de la Federación. https://www.dof.gob.mx/#gsc.tab=0
...
Instituto Nacional de Estadística y Geografía (INEGI). (1998, enero 1). Banco de indicadores. Instituto Nacional de Estadística y Geografía. INEGI. https://www.inegi.org.mx/app/indicadores/?ind=6207079644&tm=6
