...
```{r}
set.seed(123)
tamaño_de_muestra <- c(100, 300, 1000)
df <- 5
results <- data.frame(
...
for (n in tamaño_de_muestra) {
  ...
  results[i, ] <- c(n, media_muestra, varianza_muestral, varianza_teorica)
  i <- i + 1
}

kable(results, caption = "La comparación de la media y varianza teórica con las muestrales") %>%
...
set.seed(123)
n_tamaño <- 1000
muestra_tamaño <- rt(n_tamaño, df)
df_muestra <- data.frame(x = muestra_tamaño)

ggplot(df_muestra, aes(x = x)) +
  geom_histogram(aes(y = ..density..), bins = 30, fill = "mistyrose3", color = "black", alpha = 0.7) +
  stat_function(fun = dt, args = list(df = gl), color = "maroon1", size = 1.2, 
                aes(linetype = "t(5)")) +
  stat_function(fun = dnorm, args = list(mean = 0, sd = 1), color = "orchid3", size = 1.2, 
                aes(linetype = "N(0,1)")) +
  labs(
...
Y <- rchisq(1000, df = n)
T <- X / sqrt(Y / n)
hist(T, breaks = 30, probability = TRUE, main = "Histograma de T con función de densidad teórica", xlab = "T")
curve(dt(x, df = n), col = "lightsalmon", lwd = 2, add = TRUE)
...
S <- apply(X, 1, sd)
T <- (X_bar - mu) / (S / sqrt(n))
hist(T, breaks = 55, probability = TRUE, main = "Histograma de T con función de densidad teorica t(9)", xlab = "T")
curve(dt(x, df = n - 1), col = "darkslategray2", lwd = 2, add = TRUE)
...
S <- apply(X, 1, sd)
T <- (X_bar - mu) / (S / sqrt(n))
hist(T, breaks = 45, probability = TRUE, main = "Histograma de T con t(9)", xlab = "T")
curve(dt(x, df = n - 1), col = "darkorchid2", lwd = 2, add = TRUE)
