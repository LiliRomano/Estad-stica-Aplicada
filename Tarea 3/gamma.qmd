...
# Tamaños de muestra
tamaños_muestra <- c(5, 10, 20, 30, 50, 100)
set.seed(123)
...
# Genera 1000 muestras
medias <- replicate(1000, mean(rgamma(n, shape = alpha_sim, rate = lambda_sim)))
  
# estadísticas empíricas de las medias
media_emp <- mean(medias)
var_medias_emp <- var(medias)
var_medias_teorica <- varianza_teorica / n
  
# Gráfico del histograma + normal teórica
  df_plot <- tibble(medias = medias)
  p <- ggplot(df_plot, aes(x = medias)) +
    geom_histogram(aes(y = ..density..), bins = 30,
                   alpha = 0.6, fill = "steelblue") +
    stat_function(fun = dnorm,
                  args = list(mean = media_teorica,
                              sd = sqrt(var_medias_teorica)),
                  size = 1, linetype = "dashed") +
    geom_vline(xintercept = media_teorica,
               color = "red", linetype = "dotted") +
    labs(title = paste("n =", n),
         subtitle = paste0(
           "Media empírica: ", round(media_emp, 4),
           "\nMedia teórica: ", round(media_teorica, 4),
           "\nVar(emp): ", round(var_medias_emp, 5),
           " | Var(theo)/n: ", round(var_medias_teorica, 5)
         ),
...
# Mostrar los histogramas en una grilla 2x3
(plotlist <- (plots[["5"]] + plots[["10"]] + plots[["20"]]) /
              (plots[["30"]] + plots[["50"]] + plots[["100"]]))
...
lambda <- 1.5
n_reps <- 1000
n_valores <- c(5, 10, 20)  # Corregido el nombre de la variable
set.seed(456)
...
for (n in n_valores) {
sumas <- replicate(n_reps, sum(rexp(n, rate = lambda)))
df <- data.frame(sumas = sumas)
  
# Gráfico histograma mas la densidad gamma teórica
p <- ggplot(df, aes(x = sumas)) +
    geom_histogram(aes(y = after_stat(density)), bins = 30,  
                   alpha = 0.6, fill = "steelblue") +
    stat_function(fun = dgamma,
                  args = list(shape = n, rate = lambda),
                  size = 1.2, color = "red") +
    labs(title = paste("Suma de", n, "Exp(", lambda, ")"),
         subtitle = paste("Distribución gamma teórica: shape =", n, ", rate =", lambda),
         x = "Y = suma de exponenciales", y = "Densidad") +
    theme_minimal()
plots[[as.character(n)]] <- p
}

(plots[["5"]] / plots[["10"]] / plots[["20"]])
