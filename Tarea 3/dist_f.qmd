...
ggplot(df, aes(x = Y, ..density..)) +
  geom_histogram(bins = 50) +
  stat_function(fun = df, args = list(df1 = 1, df2 = n),
                linewidth = 1.2) +
  ggtitle("Inciso a) Comparación: X^2 vs F(1,n)") +
...
ggplot(df2, aes(x = Y, ..density..)) +
  geom_histogram(bins = 60) +
  stat_function(fun = df, args = list(df1 = b, df2 = a),
                linewidth = 1.2) +
  ggtitle("Inciso b) Comparación: 1/X vs F(b,a)") +
  xlab("Valores") + ylab("Densidad")
...
X <- rchisq(n_muestras, df = a)
Y <- rchisq(n_muestras, df = b)
W <- (X / a) / (Y / b)

ggplot(data.frame(W = W), aes(x = W)) +
  geom_histogram(aes(y = ..density..), bins = 100,
                 fill = "orange", color = "black", alpha = 0.6) +
  stat_function(fun = function(x) stats::df(x, df1 = a, df2 = b),
                color = "red", linewidth = 1.2) +
  ggtitle(paste("c) Verificación: (X/a)/(Y/b) ~ F(", a, ",", b, ")", sep = "")) +
  xlim(0, 5) +
  ylab("Densidad") + xlab("(X/a)/(Y/b)") +
  theme_minimal()
