...
# Porcentaje de estudiantes que miden más de 180 cm
porcentaje_mas_180 <- mean(muestra_altura > 180) * 100
porcentaje_mas_180

# Altura en el percentil 25
percentil_25 <- quantile(muestra_altura, 0.25)
percentil_25

# Histograma con densidad teórica
hist(muestra_altura, prob = TRUE, main = "Distribución de Alturas", col = "blue4")
curve(dnorm(x, mean = mu_altura, sd = sigma_altura), add = TRUE, col = "red3", lwd = 5)
...
```{r}
print("Proceso Industrial: Control de Temperatura")
# Parámetros del proceso
mu_temp <- 350
sigma_temp <- 5
lim_inf <- 340
...
cat(" 1.Simulación de un día de producción (1000 mediciones)")
set.seed(123) 
# Para resultados reproducibles
produccion_dia <- rnorm(1000, mean = mu_temp, sd = sigma_temp)

# Cálculo de porcentaje dentro de especificación
temp_dentro_espec <- pnorm(lim_sup, mu_temp, sigma_temp) - pnorm(lim_inf, mu_temp, sigma_temp)
porcentaje_dentro <- temp_dentro_espec * 100

cat(" 2.Porcentaje dentro de especificación:", round(porcentaje_dentro, 2), "%\n")

cat(" 3.Temperatura Fuera de Especificación")
porcentaje_fuera <- (1 - temp_dentro_espec) * 100
cat("Porcentaje fuera de especificación:", round(porcentaje_fuera, 2), "%\n")

cat("Efecto del Descentrado (μ = 355°C)")
mu_nueva <- 355
temp_dentro_desc <- pnorm(lim_sup, mu_nueva, sigma_temp) - pnorm(lim_inf, mu_nueva, sigma_temp)
porcentaje_dentro_desc <- temp_dentro_desc * 100

cat("Nuevo porcentaje dentro de especificación:", round(porcentaje_dentro_desc, 2), "%\n")
cat("Incremento en defectos:", round((1 - temp_dentro_desc) * 100 - porcentaje_fuera, 2), "% puntos\n")

cat(" 4.Sigma para 99.9% dentro de especificación")
objetivo <- 0.999
sigma_objetivo <- function(sigma) {
  abs((pnorm(lim_sup, mu_temp, sigma) - pnorm(lim_inf, mu_temp, sigma)) - objetivo)
}
sigma_optimo <- optimize(sigma_objetivo, interval = c(0.1, 10))$minimum

cat("Sigma requerido:", round(sigma_optimo, 3), "\n")
cat("Límite superior:", round(qnorm(0.9995, mu_temp, sigma_optimo), 2), "°C\n")
cat("Límite inferior:", round(qnorm(0.0005, mu_temp, sigma_optimo), 2), "°C\n")
...
# Simular ventas de 24 meses
set.seed(123)
ventas <- rnorm(n_meses, mu_ventas, sigma_ventas)
cat("1. Ventas simuladas (primeros 6 meses):", round(ventas[1:6], 0), "\n")

# Probabilidad ventas > 60,000
prob_60000 <- 1 - pnorm(60000, mu_ventas, sigma_ventas)
cat("2. Probabilidad ventas > 60,000:", round(prob_60000 * 100, 2), "%\n")

# Meses esperados con ventas < 30,000
prob_30000 <- pnorm(30000, mu_ventas, sigma_ventas)
meses_30000 <- prob_30000 * n_meses
cat("3. Meses esperados < 30,000:", round(meses_30000, 1), "\n")

# Rango intercuartílico
Q1 <- qnorm(0.25, mu_ventas, sigma_ventas)
Q3 <- qnorm(0.75, mu_ventas, sigma_ventas)
RIC <- Q3 - Q1
cat("4. Rango intercuartílico teórico:", round(RIC, 0), "pesos\n")

# Porcentaje meses > 45,000
prob_45000 <- 1 - pnorm(45000, mu_ventas, sigma_ventas)
cat("5. Porcentaje meses > 45,000:", round(prob_45000 * 100, 2), "%\n")
```
...
```{r}
print("Análisis de Duración de Baterías")
...
# Simular vida útil de 800 baterías
set.seed(123)
baterias <- rnorm(n_baterias, mu_bateria, sigma_bateria)
cat(" 1. Primeras 6 baterías (horas):", round(baterias[1:6], 0), "\n")

# Garantía que cubre al 95%
garantia_95 <- qnorm(0.95, mu_bateria, sigma_bateria)
cat("2. Garantía que cubre 95%:", round(garantia_95, 0), "horas\n")

# Baterías que fallan con garantía de 900 horas (10,000 unidades)
prob_falla <- pnorm(900, mu_bateria, sigma_bateria)
fallas_esperadas <- prob_falla * 10000
cat("3. Baterías que fallan (10,000):", round(fallas_esperadas, 0), "\n")

# Comparación media y mediana
media_sim <- mean(baterias)
mediana_sim <- median(baterias)
cat("4. Media simulada:", round(media_sim, 1), "vs teórica:", mu_bateria, "\n")
cat("   Mediana simulada:", round(mediana_sim, 1), "vs teórica:", mu_bateria, "\n")

# Percentil 10
percentil_10 <- qnorm(0.10, mu_bateria, sigma_bateria)
cat("5. Percentil 10:", round(percentil_10, 0), "horas\n")
```
...
:::
```{r}
print("Análisis de Errores de Medición")

# Parámetros (corrigiendo la varianza)
mu_error <- 0
sigma_error <- 2.5  # Asumiendo N(0, 2.5²)
n_mediciones <- 1000

# Simular 1000 mediciones
set.seed(123)
errores <- rnorm(n_mediciones, mu_error, sigma_error)
cat("1. Primeros 6 errores:", round(errores[1:6], 2), "\n")

# Porcentaje entre -5 y +5 unidades
porcentaje_5 <- pnorm(5, mu_error, sigma_error) - pnorm(-5, mu_error, sigma_error)
cat("2. Errores entre -5 y +5:", round(porcentaje_5 * 100, 2), "%\n")

# Probabilidad |error| > 6 unidades
prob_abs_6 <- 2 * (1 - pnorm(6, mu_error, sigma_error))
cat("3. Probabilidad |error| > 6:", round(prob_abs_6 * 100, 2), "%\n")

# Mediciones "precisas" (error ≤ 3 unidades)
prob_preciso <- pnorm(3, mu_error, sigma_error) - pnorm(-3, mu_error, sigma_error)
cat("4. Mediciones precisas (≤3):", round(prob_preciso * 100, 2), "%\n")

# Verificar simetría alrededor de 0
media_sim <- mean(errores)
cat("5. Media de errores:", round(media_sim, 4), "(debería ser ≈ 0)\n")
cat("   Proporción positivos:", round(mean(errores > 0), 3), "\n")
cat("   Proporción negativos:", round(mean(errores < 0), 3), "\n")
```
...
```{r}
print("Comparación de Resultados de Examen")
...
# Simular 200 estudiantes de cada grupo
set.seed(123)
grupo_A <- rnorm(n_estudiantes, mu_A, sigma_A)
grupo_B <- rnorm(n_estudiantes, mu_B, sigma_B)
cat("1. Primeros 5 estudiantes Grupo A:", round(grupo_A[1:5], 1), "\n")
cat("   Primeros 5 estudiantes Grupo B:", round(grupo_B[1:5], 1), "\n")

# Porcentaje Grupo A que supera media del Grupo B
porcentaje_A_supera_B <- mean(grupo_A > mu_B) * 100
cat("2. Grupo A que supera media Grupo B:", round(porcentaje_A_supera_B, 2), "%\n")

# Probabilidad Grupo B < 70
prob_B_70 <- pnorm(70, mu_B, sigma_B)
cat("3. Probabilidad Grupo B < 70:", round(prob_B_70 * 100, 2), "%\n")

# Comparación visual
hist(grupo_A, col = rgb(1, 0, 0, 0.5), main = "Comparación de Grupos", 
     xlab = "Puntuación", ylab = "Frecuencia", xlim = c(40, 100))
hist(grupo_B, col = rgb(0, 0, 1, 0.5), add = TRUE)
legend("topright", legend = c("Grupo A", "Grupo B"), 
       fill = c(rgb(1, 0, 0, 0.5), rgb(0, 0, 1, 0.5)))

# Percentil 90 de cada grupo (10% superior)
percentil_90_A <- qnorm(0.90, mu_A, sigma_A)
percentil_90_B <- qnorm(0.90, mu_B, sigma_B)
cat("5. Puntuación del 10% superior:\n")
cat("   Grupo A:", round(percentil_90_A, 1), "\n")
cat("   Grupo B:", round(percentil_90_B, 1), "\n")
```
