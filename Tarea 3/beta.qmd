...
p_media <- ggplot(df_convergencia, aes(x = n, y = media_muestral)) +
  geom_line(color = "darkseagreen2", size = 1) +
  geom_hline(yintercept = media_teo, linetype = "dashed", color = "darkolivegreen1") +
  labs(
...
p_varianza <- ggplot(df_convergencia, aes(x = n, y = varianza_muestral)) +
  geom_line(color = "lightsalmon", size = 1) +
  geom_hline(yintercept = varianza_teo, linetype = "dashed", color = "orange2") +
  labs(
...
p_hist <- ggplot(df_hist, aes(x = x)) +
  geom_histogram(aes(y = ..density..), bins = 45, fill = "palegreen", color = "black", alpha = 0.7) +
  stat_function(fun = dbeta, args = list(shape1 = a_param, shape2 = b_param), color = "palegreen4", size = 1) +
  labs(
...
for (n in tamaños_muestra_tcl) {
  ...
  p_tcl <- ggplot(df_tcl, aes(x = medias)) +
    geom_histogram(aes(y = ..density..), bins = 30, 
                   fill = "peachpuff", color = "black", alpha = 0.7) +
    stat_function(fun = dnorm, 
                  args = list(mean = a / (a + b), 
                              sd = sqrt((a * b) / ((a + b)^2 * (a + b + 1)) / n)), 
                  color = "pink2", size = 1.2) +
    labs(
      title = paste("TCL: n =", n),
      x = "Media Muestral",
      y = "Densidad"
